...
void Server::create_client (CON_TYPE type, int socket, struct sockaddr_in client_addr) {
    ...
    // Start client thread
    this->client_threads.emplace_back(&Server::handle_client, this, std::ref(this->clients.back()));
}
/***********************************************************************************/
...
/***********************************************************************************/
void Server::bind_connection (CON_TYPE type) {
    ...
    if (bind(((type == TCP) ? this->main_tcp_socket : this->main_udp_socket), addr, sizeof(this->server_addr)) < 0)
...
void Server::stop_server () {
    ...
    // Distribute BYE message to all connected clients when
...
void Server::send_message (ServerClient& to_client, DataStruct& data) {
    ...
    auto to_send = to_client.msg_queue.front();
}
...
std::vector<DataStruct> Server::handle_tcp_recv (ServerClient& client) {
    ...
    while (!client.stop_thread) {
        ...
        // Check for number of received bytes
        if (bytes_received <= 0) {
            OutputClass::out_err_intern("Unexpected client disconnected");
            session_end(client);
            break;
        }
...
void Server::handle_client (ServerClient& client) {
    while (!client.stop_thread) {
        // Begin sending (if any) (initially should be empty)
        if (!client.msg_queue.empty())
            send_data(client, client.msg_queue.front().first);
        ...
        process_client(client, recv_msgs);
    }
}

void Server::process_client (ServerClient& client, std::vector<DataStruct> recv_msgs) {
            ...
            case S_AUTH:
                switch (msg.header.type) {
                    case AUTH:
                        ...
                        if (get_client(msg.user_name, this->clients) != this->clients.end())
            ...
            case S_OPEN:
                switch (msg.header.type) {
                    case MSG:
                        ...
                        broadcast_msg(*(get_channel(client.client_channel)), std::string(client.display_name + " : " + msg.message), client.user_name);
                        break;
                    case JOIN:
